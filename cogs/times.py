        ...
        with livejson.File('times.json') as times:
            if ctx.invoked_subcommand is None:
                if len(ctx.message.mentions) == 1:
                    if ctx.message.mentions[0].id in times:
                        tz = pytz.timezone(times[ctx.message.mentions[0].id])
                        fmt = '%a %B %d %Y, %I:%M %p %Z'
                    ...
                    if ctx.message.mentions[0].id not in times and ctx.message.mentions[1].id not in times:
                    ...
                    elif ctx.message.mentions[0].id not in times:
                        await self.rasis.say('I don\'t think ' + ctx.message.mentions[0].display_name + ' has set their timezone yet. Tell them to run `;;time zones <code>` and/or `;;time set <zone>` first.')
                    elif ctx.message.mentions[1].id not in times:
                        ...
                        tz0 = pytz.timezone(times[ctx.message.mentions[0].id])
                        tz1 = pytz.timezone(times[ctx.message.mentions[1].id])
                        fmt = "%z"
